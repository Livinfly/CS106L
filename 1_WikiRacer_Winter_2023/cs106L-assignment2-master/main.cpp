...
int numCommonLinks_1_in_2(const unordered_set<string>& unst1, const unordered_set<string>& unst2) {
    return count_if(unst1.begin(), unst1.end(), [&unst2](auto x) { return unst2.count(x);});
}
int numCommonLinks(const unordered_set<string>& curr_set, const unordered_set<string>& target_set) {
    // replace all of these lines!
    if(curr_set.size() < target_set.size()) return numCommonLinks_1_in_2(curr_set, target_set);
    else return numCommonLinks_1_in_2(target_set, curr_set);
    // (void) target_set;
    // (void) curr_set;
    // return 0; 
}
...
vector<string> findWikiLadder(const string& start_page, const string& end_page) {
    ...
    auto cmp_fn = [&w, &target_set](const vector<string>& left, const vector<string>& right) {
        // replace all of these lines.
        int cntl = numCommonLinks(w.getLinkSet(left.back()), target_set), cntr = numCommonLinks(w.getLinkSet(right.back()), target_set);
        return cntl < cntr;
        // (void) w;
        // (void) target_set;
        // (void) left;
    ...
    std::priority_queue<vector<string>, std::vector<vector<string>>, decltype(cmp_fn)> queue(cmp_fn);
