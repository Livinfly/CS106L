...
bool valid_wikilink(const string& link) {
    // replace these lines!
    return std::all_of(link.begin(), link.end(), [](auto x){ return x != ':' && x != '#';});
    // (void) link;
...
unordered_set<string> findWikiLinks(const string& inp) {
    ...
    while(true) {
        ...
        // throw std::invalid_argument("Not implemented yet.\n");
        url_start = std::search(url_start+(url_start!=inp.begin()), end, delim.begin(), delim.end());
        ...
        auto url_end = find(url_start+delim.size(), end, '\"');
        // END STUDENT CODE HERE
        ///////////////////////////////////////////////////////////////////////////////////////////////////
...
unordered_set<string> WikiScraper::getLinkSet(const string& page_name) {
    // std::cerr << "getLinkSet Begin" << endl;
...
string WikiScraper::getPageSource(const string &page_name) {
    ...
    if(page_cache.find(page_name) == page_cache.end()) {
        ...
        int cnt = 0;
        while(r.status_code != 200 && cnt++ < 10) {
            std::cerr << "try " << page_name << ' ' << cnt << endl;
            r = cpr::Get(cpr::Url{url});
        }
        string ret = r.text;
        // std::cerr << r.status_code << endl;
        // std::cerr << ret << endl;
